...
```bash
nextflow pull mpozuelo/MGI_demux
```
...
First, go to the [mpozuelo/MGI_demux releases page](https://github.com/mpozuelo/MGI_demux/releases) and find the latest version number - numeric only (eg. `1.0`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.0`.
